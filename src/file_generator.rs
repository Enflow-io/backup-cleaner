...
pub fn generate_daily_files(files_qnt: i64, mayby_path: Option<&str>) -> IoResult<()> {
    let root_path =
        std::env::var("CARGO_MANIFEST_DIR").unwrap_or_else(|_| panic!("Can't get root path"));
    ...
    for i in 0..files_qnt {
        ...
        if double_create {
            ...
            let mut file = File::create(&second_file_name)?;
...
pub fn generate_weekly_files(files_qnt: i64) -> IoResult<()> {
    let path = std::env::var("CARGO_MANIFEST_DIR").expect("Can't get root path");
    // let path = match std::env::var("CARGO_MANIFEST_DIR") {
    //     Ok(path) => path,
    //     Err(_) => panic!("Can't get root path"),
    // };
    ...
    for i in 0..files_qnt {
        ...
        if double_create {
            let second_file_name =
                format!("{path}/test-data/{day_str}.{month_str}.{year}_{i}_2.tar");
